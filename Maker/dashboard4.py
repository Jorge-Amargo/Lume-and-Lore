    ...
    if st.session_state.current_step == "narrative":
        ...
        if st.session_state.scene_data is None:
            with st.spinner("🕵️ Architect is drafting..."):
                ...
                if data is None:
                    st.error("The Architect failed to generate a valid scene. Please try hitting 'Start' or 'Resume' again.")
                    st.stop() # Stops execution before the crash happens
            
                # Ensure semantic IDs exist
