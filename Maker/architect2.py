...
class AutonomousArchitect:
    def __init__(self, book_path):
        ...
        Loads configuration and establishes the model name dynamically.
        """
        ...
        # FIX 1: Read model from config right away so we use it everywhere
    ...
    def _setup_context_cache(self):
        """
        Manages the Google Context Cache. 
        Deletes existing caches for this book if the model has changed.
        """
        print(f"🌀 Initializing Cache with {self.model_name}...")
        cache_name = f"cache-{self.book_id}".lower().replace("_", "-")
        model_full_path = f"models/{self.model_name}"
        
        # 1. Look for existing caches on Google's servers
        existing_caches = list(caching.CachedContent.list())
        for c in existing_caches:
            # If we find a cache for this specific book project
            if c.display_name == f"Context_{self.book_id}":
                # Check if it's tied to a different model
                if c.model != model_full_path:
                    print(f"🗑️ Deleting incompatible cache (Model Mismatch: {c.model} vs {model_full_path})")
                    c.delete()
                else:
                    # Cache is valid and matches model! Reuse it.
                    print(f"♻️ Reusing existing cache for {self.book_id}")
                    return c

        # 2. If no valid cache was found/kept, create a new one
        print(f"📤 Uploading new context cache for '{self.book_id}' using {self.model_name}...")
        ...
        new_cache = caching.CachedContent.create(
            ...
            display_name=f"Context_{self.book_id}",
            contents=[book_content],
            ttl=datetime.timedelta(hours=1), # Cache lasts 1 hour of inactivity
        )
        return new_cache
    
    def initialize_engine(self, skip_intro=False):
        # Uses self.book_path which was set in __init__
        print(f"📚 Architect reading source text: {self.book_path}...")
        
        with open(self.book_path, "r", encoding="utf-8") as f:
            book_text = f.read()
            
        # The cache display name should use the ID from the config we loaded
        ...
        # FIX 2: Clear old mismatched caches first to avoid INVALID_ARGUMENT errors
        print(f"🧹 Checking for old caches for {self.model_name}...")
        try:
            for c in self.client.caches.list():
                # If a cache exists with the same display name but potentially a different model, delete it
                if c.display_name == f"engine_{self.config['book_id']}":
                    print(f"🗑️ Deleting existing cache: {c.name}")
                    self.client.caches.delete(name=c.name)
        except Exception as e:
            print(f"ℹ️ Cache cleanup skipped: {e}")

        # FIX 3: Create the Cache using self.model_name (not the hardcoded constant)
        print(f"📤 Creating new cache using {self.model_name}...")
        try:
        ...
        # FIX 4: Use the SAME model name for the chat session
        self.chat = self.client.chats.create(
            model=self.model_name, # <--- MUST MATCH CACHE
