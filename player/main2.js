...
function isSceneTransition(tags) {
    ...
    // It's a scene transition if there's an image that ISN'T a reward or item
    return tags.some(t => t.toUpperCase().includes('IMAGE:') && 
                         !t.toLowerCase().includes('_reward') && 
                         !t.toLowerCase().includes('_item'));
}
// --- GAME ENGINE ---
...
function continueStory() {
    if (!story) return;
    choicesContainer.innerHTML = ""; // Clear "Proceed" button if present
    
    while (story.canContinue) {
        const text = story.Continue();
        const tags = story.currentTags;
        
        // Safety: If we hit another scene transition inside the loop, we should technically stop.
        // But for now, let's assume one scene per chunk.
    ...
    requestAnimationFrame(() => {
        ...
        if (textPane) {
            textPane.scrollTo({ top: 0, behavior: 'auto' }); // Jump to top for new scene
        }
    });
}
